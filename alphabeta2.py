...
class PacmanAgent(Agent):
    ...
    def _max_value(self, gameState)->(float, doc.GameState):
        ...
        for (succGameState, succAction) in \
            ...
            if v >= self._beta :
                print("v was >= beta in max value")
                return v

            self._alpha = max(self._alpha, v)
    ...
    def _min_value(self, gameState)->(float, doc.GameState):
        ...
        for (succGameState, succAction) in \
            ...
            if v <= self._alpha:
                print("v was <= alpha in min value : v = {}".format(v))
                return v

            self._beta = min(self._beta, v)
            print("beta = {} in min value ".format(self._beta))
